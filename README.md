...
- Build backend: Gradle 8.14.5, Gradle Kotlin DSL (`*.gradle.kts`). Mã nguồn Java nằm hoàn toàn trong `src/main/java`.
...
## Vai trò sản phẩm & Tài khoản Demo

Mỗi tài khoản chỉ có đúng một vai trò:

| Vai trò | Chức năng chính | Tài khoản demo (`username` / `password`) |
|---|---|---|
| `ADMIN` | Quản trị tài khoản, tổ chức, thương hiệu, dịch vụ ngoài, báo cáo, giấy phép và vận hành. | `admin` / `admin123` |
| `INSTRUCTOR` | Biên soạn khóa học, tài liệu, bài thực hành, bài kiểm tra trong khóa học, bài thi độc lập và chấm điểm. | `instructor` / `admin123` |
| `STUDENT` | Học khóa được giao, xem video/PDF/DOCX, nộp bài thực hành, làm bài kiểm tra/bài thi và xem kết quả. | `student` / `admin123` |
...
## Điểm mới trong 0.24.0

- Sửa trực tiếp giao diện React theo bộ thiết kế trắng–tím: trang Tổ chức có sơ đồ cơ cấu ngang và vùng chi tiết tách biệt; danh sách bài thi dùng các thẻ độc lập; trình soạn đề dùng ba cột thật; màn hình làm bài chọn bằng ô A/B/C/D thay cho vòng tròn.
...
- Docker build một backend bundle dùng chung, khởi động có health dependency và đóng gói tự động lên GitHub Container Registry (GHCR).
...
```text
.github/                          CI/CD Workflows (GitHub Actions & GHCR Docker publish)
...
scripts/                          Validator và operations agent
```
...
Hoặc dùng script tự động chờ hệ thống sẵn sàng:
...
Lần đầu, Ollama được khởi động nhưng chưa tải trọng số model. Đăng nhập Admin (`admin` / `admin123`) → **Cài đặt → Kết nối model AI** và nhấn **Tải và tự thiết lập**. Model được giữ trong Docker volume nên những lần sau không tải lại.

Các profile tùy chọn:

```bash
docker compose --profile extended up -d       # integration-service
...
Hệ thống hỗ trợ tự động build và push các pre-built image lên **GitHub Container Registry (GHCR)** qua GitHub Actions:
...
4. Thay đổi API phải cập nhật controller/DTO, contract và consumer test.
