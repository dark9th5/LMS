# Ma trận truy vết BA + yêu cầu mở rộng — LMSPilot CLS 0.8.0
...
| UC-RBAC-01 | Role tùy chỉnh, bulk grant/revoke, `ALLOW/DENY`, scope SYSTEM/ORG/COURSE/CLASS/EXAM | IMPLEMENTED | Token/session invalidation policy phải UAT |
...
| UC-CONTENT-01 | Chương/bài, file đa định dạng, inline viewer, file version | IMPLEMENTED | SCORM import/player chưa phải baseline; xAPI LRS đã có |
...
| UC-LEARN-02 | Server progress, position, time spent, completion event, xAPI | IMPLEMENTED | Offline sync ngoài LAN không trong baseline |
...
| Mã/yêu cầu | Hiện thực | Trạng thái | Còn lại |
|---|---|---|---|
| UC-COMP-01 | Completion policy/event và progress read model | IMPLEMENTED | UAT từng policy |
...
| Security | Password hash, scoped RBAC, service token, file validation, secret encryption | TLS, at-rest encryption, threat model, SAST/DAST/pentest, malware adapter |
...
## 7. Xác minh bản 0.8.0

- Repository validator: 10 JSON, 28 YAML, 19 service, 18 Flyway service — PASS.
- Static/contract/unit suite: **73/73 PASS**.
- TypeScript/TSX parser và shell syntax: PASS.
- Archive cuối được giải nén và chạy lại validator/test.
- Gradle/npm/Docker E2E chưa thể chạy trong môi trường tạo bản; xem `BUILD_VERIFICATION_0.8.0.md`.
