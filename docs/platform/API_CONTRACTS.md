# LMSPilot — API contracts đang triển khai

Tài liệu này mô tả các endpoint hiện có trong source tree của bản LMSPilot. Tất cả API quản trị đều yêu cầu JWT và permission tương ứng; các API `/internal/v1/**` yêu cầu internal service token và không dành cho trình duyệt.
...
### Giao khóa học và ghi danh

- `GET /api/v1/enrollments/me`
- `POST /api/v1/course-assignments`
- `GET /api/v1/course-assignments`
- `DELETE /api/v1/course-assignments/{id}`
- `GET /api/v1/course-assignments/me`

Course assignment hỗ trợ đích `USER`, `GROUP`, `DEPARTMENT`, `BRANCH`, thời điểm bắt đầu, hạn hoàn thành, grace period và bắt buộc/tùy chọn. Sản phẩm 0.20 không có API hoặc giao diện Lớp học công khai; bản ghi delivery/class cũ chỉ còn là chi tiết tương thích nội bộ để ghim phiên bản khóa học.

### Buổi học trực tuyến trong khóa học
...
Provider hỗ trợ local endpoint hoặc API bên ngoài do khách hàng cấu hình. API key được mã hóa bằng `AI_SECRET_KEY`. Kết quả phải tuân theo `contracts/lmspilot/question-set.schema.json`, qua schema validation, business validation và bước review trước khi import vào ngân hàng câu hỏi.
