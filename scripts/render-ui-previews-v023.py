#!/usr/bin/env python3
"""Render deterministic LMSPilot 0.23.0 visual QA previews.
...
OUT = ROOT / "docs" / "screenshots" / "0.23.0"
TMP = ROOT / ".preview-render-v023"
...
def _with_overlay(page: str, overlay: str) -> str:
    return page.replace("</body>", overlay + "</body>")


def create_course_modal_page() -> str:
    body = dashboard("INSTRUCTOR")
    overlay = '''<div class="modal-backdrop"><section class="modal modal-wide" role="dialog" aria-modal="true"><header class="modal-header"><div><h2>Tạo khóa học mới</h2><p>Thông tin được chia nhóm rõ ràng; phần nội dung cuộn độc lập và nút lưu luôn nhìn thấy.</p></div><button class="icon-button">×</button></header><div class="modal-body"><form class="entity-form"><div class="form-grid"><label class="wide"><span>Tên khóa học</span><input value="Kỹ năng số nền tảng"></label><label><span>Mã khóa học</span><input value="DIGITAL-FOUNDATION"></label><label><span>Danh mục</span><select><option>Kỹ năng chuyên môn</option></select></label><label class="wide"><span>Mô tả ngắn</span><textarea>Khóa học giúp học viên xây dựng năng lực số, an toàn dữ liệu và kỹ năng cộng tác.</textarea></label><label><span>Thời lượng dự kiến</span><input value="12 giờ"></label><label><span>Ngôn ngữ</span><select><option>Tiếng Việt</option></select></label><label><span>Điều kiện hoàn thành</span><select><option>Hoàn thành nội dung bắt buộc</option></select></label><label><span>Trạng thái ban đầu</span><select><option>Bản nháp</option></select></label><label class="wide"><span>Ảnh bìa</span><input value="Chưa chọn tệp"></label></div><footer class="modal-actions entity-form-actions"><button class="button secondary">Hủy</button><button class="button primary">Tạo khóa học</button></footer></form></div></section></div>'''
    return _with_overlay(body, overlay)


def create_exam_modal_page() -> str:
    body = exam_overview_page()
    overlay = '''<div class="modal-backdrop"><section class="modal modal-wide" role="dialog" aria-modal="true"><header class="modal-header"><div><h2>Tạo bài thi độc lập</h2><p>Thiết lập thông tin, thời gian và chính sách làm bài trước khi chọn câu hỏi.</p></div><button class="icon-button">×</button></header><div class="modal-body"><form class="entity-form"><div class="form-grid"><label class="wide"><span>Tên bài thi</span><input value="Kỳ thi năng lực số 2026"></label><label><span>Mã bài thi</span><input value="EX2026-001"></label><label><span>Thời lượng</span><input value="90 phút"></label><label><span>Thời gian mở</span><input value="20/05/2026 08:00"></label><label><span>Thời gian đóng</span><input value="27/05/2026 23:59"></label><label><span>Số lần làm tối đa</span><input value="2"></label><label><span>Điểm đạt</span><input value="60%"></label><label><span>Xáo trộn câu hỏi</span><select><option>Có</option></select></label><label class="wide"><span>Hướng dẫn cho thí sinh</span><textarea>Đọc kỹ câu hỏi, hệ thống tự động lưu đáp án và chỉ nộp khi đã kiểm tra.</textarea></label></div><footer class="modal-actions entity-form-actions"><button class="button secondary">Hủy</button><button class="button primary">Tiếp tục chọn câu hỏi</button></footer></form></div></section></div>'''
    return _with_overlay(body, overlay)


def ai_provider_modal_page() -> str:
    body = ai_connection_quick_page()
    overlay = '''<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true"><header class="modal-header"><div><h2>Kết nối model AI</h2><p>Cấu hình endpoint OpenAI-compatible; API key chỉ được gửi tới backend.</p></div><button class="icon-button">×</button></header><div class="modal-body"><form class="entity-form"><div class="form-grid"><label class="wide"><span>Tên cấu hình</span><input value="AI sản xuất"></label><label class="wide"><span>Loại kết nối</span><select><option>API key riêng</option></select></label><label class="wide"><span>Base URL</span><input value="https://api.example.com/v1"></label><label><span>Model</span><input value="model-name"></label><label><span>Timeout</span><input value="180 giây"></label><label class="wide"><span>API key</span><input type="password" value="••••••••••••••"></label></div><footer class="modal-actions entity-form-actions"><button class="button secondary">Kiểm tra kết nối</button><button class="button primary">Lưu và bật</button></footer></form></div></section></div>'''
    return _with_overlay(body, overlay)
...
SCREENS: list[tuple[str, Callable[[], str], tuple[int,int]]] = [
    ...
    ("23-create-course-modal.png", create_course_modal_page, (1440,900)),
    ("24-create-exam-modal.png", create_exam_modal_page, (1440,900)),
    ("25-ai-provider-modal.png", ai_provider_modal_page, (1440,900)),
    ("26-short-viewport-modal.png", create_course_modal_page, (1280,600)),
]
