#!/usr/bin/env python3
"""Render deterministic LMSPilot 0.20.1 polished role-separated UI previews.
...
OUT = ROOT / "docs" / "screenshots" / "0.20.1"
TMP = ROOT / ".preview-render-v0201"
...
def shell(role: str, active: str, content: str, *, dark: bool = False) -> str:
    ...
    <div class="app-shell role-{role.lower()}" data-portal-role="{role}">
      <aside class="app-sidebar">
        <div class="sidebar-brand-row"><div class="app-brand"><span class="brand-mark">L</span><span class="brand-copy"><strong>LMSPilot</strong></span></div></div>
...
def users_page() -> str:
    rows = [
        ("QT","Quản trị hệ thống","admin@lmspilot.local","Quản trị viên","Đang hoạt động"),
        ("MA","Nguyễn Minh Anh","minh.anh@lmspilot.local","Giảng viên","Đang hoạt động"),
        ("HN","Trần Hải Nam","hai.nam@lmspilot.local","Học viên","Đang hoạt động"),
        ("TH","Phạm Thu Hà","thu.ha@lmspilot.local","Giảng viên","Đang hoạt động"),
        ("QB","Lê Quốc Bảo","quoc.bao@lmspilot.local","Học viên","Tạm khóa"),
    ]
...
def login_page() -> str:
    return f'''<!doctype html><html lang="vi" data-theme="unified-light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>{{CSS}}</style><style>{{PREVIEW_CSS}}</style></head><body><main class="auth-page"><section class="auth-showcase custom-login-bg"><div class="auth-brand"><span class="auth-brand-mark">L</span><div><strong>LMSPilot</strong><small>Không gian học tập của tổ chức</small></div></div><div class="auth-showcase-copy"><span class="auth-pill">{icon('learn',16)} Học tập liền mạch</span><h2>Mỗi ngày tiến thêm một bước.</h2><p>Khách hàng có thể tải ảnh nền riêng; hệ thống tự phủ gradient để logo và nội dung luôn dễ đọc.</p></div><div class="auth-preview"><article class="auth-course-card auth-course-primary"><span class="auth-course-icon">{icon('book',22)}</span><div><small>Đang học</small><strong>Kỹ năng số nền tảng</strong></div><span class="auth-progress-value">72%</span><div class="auth-progress-track"><i></i></div></article><article class="auth-course-card auth-course-secondary"><span class="auth-course-icon">{icon('exam',22)}</span><div><small>Sắp tới</small><strong>Kỳ thi năng lực số</strong></div><span class="auth-date">09:30</span></article></div><p class="auth-privacy">{icon('lock',16)} Dữ liệu được bảo vệ trong hệ thống của tổ chức.</p></section><section class="auth-access"><div class="auth-card"><header class="auth-card-heading"><span class="auth-card-icon">{icon('lock',22)}</span><div><p class="auth-eyebrow">Chào mừng trở lại</p><h1>Đăng nhập</h1><p>Đăng nhập bằng đúng tài khoản Admin, Giảng viên hoặc Học viên.</p></div></header><form class="auth-form"><label class="field-group"><span>Tên đăng nhập hoặc email</span><span class="input-shell">{icon('users',19)}<input value="hocvien@lmspilot.local"></span></label><label class="field-group"><span>Mật khẩu</span><span class="input-shell">{icon('lock',19)}<input type="password" value="password"></span></label><button class="button primary auth-submit">Đăng nhập {icon('arrow',18)}</button></form><div class="role-login-note"><span>Admin</span><span>Giảng viên</span><span>Học viên</span></div></div></section></main></body></html>'''
