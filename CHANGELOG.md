...
- Viết lại README với chức năng, API, database, service và port.
- Thêm tài liệu phân công service và README riêng cho từng service.
- Không thay đổi nghiệp vụ của 0.20.3.

## 0.20.3 - 2026-08-05

- Cho giảng viên chọn phân bố độ khó khi AI sinh câu hỏi từ PDF/DOCX: Cơ bản, Cân bằng, Nâng cao hoặc tùy chỉnh.
- Chuyển luồng AI sang sinh bản nháp → xem trước từng câu → chọn câu → duyệt → nhập; không còn tự động duyệt và nhập thẳng.
- Backend bắt buộc tổng tỷ lệ bằng 100%, tính số câu theo largest remainder và kiểm tra chính xác số câu Dễ/Trung bình/Khó.
- Kiểm tra câu trùng, phương án trùng, loại câu hỏi, lời giải và trích dẫn nguyên văn; tự yêu cầu model sửa lại một lần nếu kết quả chưa đạt.
- Tách nội dung PDF theo trang khi parser cung cấp page break, DOCX theo phần; lưu trang/mục nguồn trong bản xem trước.
- Bổ sung bộ kiểm thử OpenAI-compatible local mock với PDF 2 trang và DOCX tiếng Việt, gồm kiểm thử âm cho sai phân bố và trích dẫn giả.

## 0.20.1 - 2026-08-05

- Giữ nguyên mô hình ba vai trò và kiến trúc course-only của 0.20.0.
- Khôi phục nhịp thiết kế thoáng, tinh tế của giao diện 0.18.
- Bỏ khối vai trò chiếm chỗ trong sidebar; vai trò vẫn hiển thị ở hồ sơ và topbar.
- Cân lại hero, tiêu đề, thẻ, bảng, biểu mẫu, course editor, quiz builder và learning player.
- Tăng độ rõ chữ nhỏ và tinh chỉnh dark mode không dùng viền trắng.

## 0.20.0 - 2026-08-05

- Chuyển sản phẩm về đúng ba role độc quyền `ADMIN`, `INSTRUCTOR`, `STUDENT`; một tài khoản chỉ có đúng một role và mỗi role dùng namespace giao diện riêng.
- Tách menu, dashboard, route và thao tác của ba role; truy cập nhầm role bị chuyển về cổng sở hữu, API backend vẫn kiểm tra permission.
- Loại bỏ route/giao diện Lớp học; người học được gán trực tiếp vào Khóa học. Bản ghi phân phối nội bộ chỉ còn để tương thích migration.
- Đưa bài kiểm tra vào chi tiết Khóa học; `COURSE_QUIZ` bắt buộc gắn bài học loại EXAM. Mục Bài thi chỉ còn kỳ thi độc lập.
- Hoàn thiện trình học video, PDF, DOCX, tệp và bài thực hành; học viên tải bài lên bằng purpose `ASSIGNMENT_SUBMISSION`.
- Giới hạn tạo câu hỏi AI vào PDF/DOCX thuộc đúng khóa học, có bước duyệt trước khi nhập ngân hàng câu hỏi.
- Cho ADMIN tải ảnh nền đăng nhập và logo; giới hạn purpose tải tệp theo từng role.
- Giữ style 0.18 dễ đọc, tăng chữ nhỏ và áp dụng đường viền xanh-xám trầm cho dark mode thay vì viền trắng.
- Chuẩn hóa 19 service thành thư mục độc lập, port riêng, README ownership và service catalog để dễ chia việc.

## 0.18.0 - 2026-08-05

- Rút sidebar về nhãn một dòng, thu gọn được và ghi nhớ trạng thái; loại bỏ phần mô tả lặp lại dưới từng mục.
- Hợp nhất Khóa học và Lớp học thành một khu vực Khóa học với hai tab nội dung và lớp triển khai; route cũ được chuyển hướng an toàn.
- Thiết kế lại Tổ chức theo bố cục cây cố định + vùng thao tác rộng, ghi nhớ đơn vị đang chọn và cho chọn thành viên theo tên/email thay vì nhập ID.
- Thay Theme Studio trong Cài đặt bằng Cấu hình thông tin: tải logo thật, tên thương hiệu, giới thiệu ngắn, tên miền và bộ màu chủ đạo; sáng/tối tiếp tục là lựa chọn cá nhân trên topbar.
- Viết lại Dịch vụ ngoài bằng biểu mẫu theo loại cho Redis, SMTP, AI, S3, ONLYOFFICE và họp trực tuyến; thêm tài liệu chính thức, chỉnh sửa, kiểm tra kết nối và validation backend.
- Bổ sung quyền tải/đọc tệp cho gói PLATFORM_CUSTOMIZER để luồng logo hoạt động đúng; giữ bí mật mã hóa AES-GCM.
- Python regression suite có 122 test đạt, 34 test môi trường được skip (156 test tổng cộng); repository validator đạt. npm/Gradle đầy đủ cần chạy trong CI có registry và Gradle distribution.
...
- Full npm/Gradle/Docker verification chưa chạy được trong môi trường đóng gói; xem `TEST_RESULTS_LMSPILOT_0.15.0.md`.
...
- Thêm ma trận truy vết BA+LMSPilot và operations runbook; giữ trạng thái release candidate do chưa chạy full build/Docker E2E trong môi trường tạo bản.
