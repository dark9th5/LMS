...
class ClsV080RequirementTests(unittest.TestCase):
    def test_learning_paths_pin_versions_and_backend_remains_available(self) -> None:
        ...
        self.assertNotIn('/learning-paths', shell)
        self.assertNotIn('"learning-paths"', route)
    ...
    def test_kpi_queries_are_bounded_by_assigned_classes_and_reminder_errors_are_client_errors(self) -> None:
        ...
        self.assertIn("enrollmentScope.assignedDeliveryIds(CurrentUser.id())", kpi)
        self.assertIn("readModels.findAllByClassIdIn(classIds)", kpi)
        self.assertIn('CurrentUser.hasRole("ADMIN")', kpi)
    ...
    def test_notification_automation_is_permissioned_audited_and_retired_from_core_ui(self) -> None:
        ...
        self.assertNotIn('"notification-automation"', route)
        self.assertIn("if (!target) notFound()", route)
