...
export async function POST(request: Request) {
  ...
  const upstream = await fetchGateway("/api/v1/auth/change-password", {
    method: "POST",
    headers: {
  ...
  if (!upstream.ok) {
    const error = await upstream
      .json()
      .catch(() => ({ message: "Không thể đổi mật khẩu" }));
    return NextResponse.json(error, { status: upstream.status });
  }
  const response = NextResponse.json({ ok: true });
  for (const name of ["lmspilot_access", "lmspilot_refresh", "lmspilot_user"])
