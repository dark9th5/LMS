...
export function ClassesPage({ user }: { user: PortalUser }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Không thể tải lớp đào tạo",
      );
    } finally {
      setLoading(false);
    }
  ...
  const classList = useMemo(
    () =>
      Array.isArray(classes)
        ? classes
        : unwrapItems<TrainingClass>(classes as any),
    [classes],
  );
  ...
  const filtered = useMemo(
    () =>
      classList.filter((item) =>
        `${item.name} ${item.code} ${courseMap.get(item.courseId)?.name ?? ""}`
          .toLowerCase()
          .includes(query.trim().toLowerCase()),
      ),
    [classList, courseMap, query],
  );
  ...
  async function createClass(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const dateValue = (name: string) =>
        data.get(name)
          ? new Date(`${String(data.get(name))}T00:00:00`).toISOString()
          : null;
      const created = await apiRequest<TrainingClass>("/api/v1/classes", {
        method: "POST",
        body: JSON.stringify({
          ...
          dueAt: data.get("dueAt")
            ? new Date(`${String(data.get("dueAt"))}T23:59:59`).toISOString()
            : null,
          instructorIds: [String(data.get("instructorId") ?? "")],
        }),
      });
      setOpen(false);
      setToast("Đã mở lớp đào tạo. Tiếp theo hãy ghi danh học viên.");
      await load();
      window.setTimeout(
        () => window.location.assign(`/classes/${created.id}`),
        450,
      );
    } catch (caught) {
      setFormError(
        caught instanceof Error ? caught.message : "Không thể tạo lớp",
      );
    } finally {
      setSaving(false);
    }
    ...
    <>
      ...
      <section className="summary-row">
        ...
        <div className="summary-card">
          ...
          <div>
            <span>Đang mở</span>
            <small>Cho phép ghi danh và học tập</small>
          </div>
          <strong>
      ...
      <section className="toolbar-card">
        <label className="search-field">
          ...
          <input
            ...
            placeholder="Tìm lớp, mã lớp hoặc khóa học"
          />
        </label>
      ...
      {loading ? (
        ...
        <EmptyState
          title="Chưa có lớp đào tạo"
          description="Xuất bản ít nhất một khóa học, sau đó mở lớp và ghi danh học viên."
          action={
            canCreate ? (
              <button className="button primary" onClick={() => setOpen(true)}>
                <Icon name="plus" />
        ...
        <section className="data-list class-atlas">
      ...
      <Modal
        ...
        description="Chỉ khóa học đã xuất bản mới có thể dùng để mở lớp."
      >
        <form className="form-stack" onSubmit={createClass}>
          <div className="form-grid two">
            <label>
              Mã lớp <b>*</b>
              <input name="code" required placeholder="CLASS-2026-01" />
            </label>
            <label>
              Giảng viên <b>*</b>
          ...
          <label>
            Tên lớp <b>*</b>
            <input name="name" required placeholder="Tên đợt đào tạo" />
          </label>
          <label>
            Khóa học đã xuất bản <b>*</b>
          ...
          {instructors.length === 0 && (
            <div className="form-alert info">
              <Icon name="warning" />
              Chưa có tài khoản giảng viên khả dụng.
            </div>
          )}
          {formError && (
