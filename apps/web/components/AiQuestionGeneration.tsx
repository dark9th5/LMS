...
const PRESETS: Array<{
...
}> = [
  {
    key: "BASIC",
    label: "Cơ bản",
    hint: "60% dễ · 30% trung bình · 10% khó",
    value: { EASY: 60, MEDIUM: 30, HARD: 10 },
  },
  {
    key: "BALANCED",
    label: "Cân bằng",
    hint: "30% dễ · 50% trung bình · 20% khó",
    value: { EASY: 30, MEDIUM: 50, HARD: 20 },
  },
  {
    key: "ADVANCED",
    label: "Nâng cao",
    hint: "10% dễ · 40% trung bình · 50% khó",
    value: { EASY: 10, MEDIUM: 40, HARD: 50 },
  },
];
...
export function allocateQuestionCounts(
  ...
  const raw = keys.map((key) => ({
    key,
    exact: (Math.max(0, total) * distribution[key]) / 100,
  }));
  const result = {
    EASY: 0,
    MEDIUM: 0,
    HARD: 0,
  } satisfies DifficultyDistribution;
  raw.forEach(({ key, exact }) => {
    result[key] = Math.floor(exact);
  });
  let remaining =
    Math.max(0, total) - keys.reduce((sum, key) => sum + result[key], 0);
  raw
    .map((item) => ({ ...item, fraction: item.exact - Math.floor(item.exact) }))
    .sort(
      (a, b) =>
        b.fraction - a.fraction || keys.indexOf(a.key) - keys.indexOf(b.key),
    )
    .forEach(({ key }) => {
      if (remaining > 0) {
        result[key] += 1;
...
export function DifficultyDistributionSelector({
  ...
  const counts = useMemo(
    () => allocateQuestionCounts(numberOfQuestions, value),
    ...
    <section className="ai-difficulty-card" aria-label="Phân bố độ khó câu hỏi">
      <div className="section-title compact-title">
        <div>
          ...
          <p>
            Chọn cấu hình nhanh hoặc tự điều chỉnh. Tổng tỷ lệ phải bằng 100%.
          </p>
        </div>
        <span className={`ai-total ${total === 100 ? "valid" : "invalid"}`}>
          Tổng {total}%
        </span>
      </div>
      <div className="ai-difficulty-presets">
        {PRESETS.map((preset) => {
          const selected = (Object.keys(preset.value) as DifficultyKey[]).every(
            (key) => preset.value[key] === value[key],
          );
          return (
            <button
              type="button"
      ...
      <div className="ai-difficulty-grid">
        {(
          ...
          ] as Array<[DifficultyKey, string, string]>
        ).map(([key, label, hint]) => (
          <label
            className={`ai-difficulty-row difficulty-${key.toLowerCase()}`}
            key={key}
          >
            <span>
              <strong>{label}</strong>
            ...
            <span className="ai-percent-input">
              <input
                type="number"
                min="0"
                max="100"
                value={value[key]}
                onChange={(event) =>
                  update(key, Number(event.target.value) || 0)
                }
              />
              <b>%</b>
            </span>
  ...
  return (
    <div className="ai-review-layout">
      <div className="ai-review-toolbar">
        ...
        >
          <Icon name={selectedAll ? "close" : "check"} />{" "}
      ...
      {job.validationProblems?.length ? (
        <div className="form-alert error">
          <Icon name="warning" />
          {job.validationProblems.map((problem) => problem.message).join(" · ")}
        </div>
      ) : null}
      <div className="ai-review-list">
        {questions.map((question, index) => {
          ...
          return (
            <article
              className={`ai-review-question ${selected ? "selected" : ""}`}
            ...
            >
              <label className="ai-review-select">
                <input
                  type="checkbox"
                  checked={selected}
                  onChange={() => onToggle(question.externalId)}
                />
                <span>{index + 1}</span>
              </label>
              <div className="ai-review-content">
                ...
                <div className="ai-review-question-head">
                  {editingIds.includes(question.externalId) ? (
                    <textarea
                      value={question.stem}
                      onChange={(event) =>
                        updateQuestion(question, { stem: event.target.value })
                      }
                    ...
                    <button
                      ...
                      onClick={() =>
                        setEditingIds((current) =>
                          current.includes(question.externalId)
                            ? current.filter((id) => id !== question.externalId)
                            : [...current, question.externalId],
                        )
                      }
                    >
                      <Icon name="edit" />{" "}
                ...
                {editingIds.includes(question.externalId) && (
                  <label className="ai-review-difficulty-edit">
                    Độ khó
                    <select
                ...
                <div className="ai-review-options">
                  {question.options.map((option) => (
                    <div
                      className={correct.has(option.id) ? "correct" : ""}
                      key={option.id}
                    >
                      <button
                        type="button"
                        className="ai-answer-key"
                        onClick={() =>
                          editingIds.includes(question.externalId) &&
                          toggleCorrect(question, option.id)
                        }
                        aria-label={`Đặt ${option.id} là đáp án đúng`}
                      >
                        {option.id}
                      </button>
                      {editingIds.includes(question.externalId) ? (
