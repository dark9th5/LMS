...
export function DocxPreview({
  ...
  if (error) {
    return (
      <div className="resource-viewer docx">
    ...
    <div className={`resource-viewer docx ${compact ? "compact" : ""}`}>
      <header className="docx-preview-header">
        ...
        <div>
          ...
          <small>Xem trực tiếp trong khóa học</small>
        </div>
        <a className="button secondary compact" href={downloadUrl}>
          <Icon name="download" />
          Tải DOCX
      ...
      <article
        className="docx-page"
        aria-label={`Nội dung ${preview.originalName}`}
      >
        {preview.paragraphs.length ? (
          ...
          <p>Tài liệu không có đoạn văn bản có thể hiển thị.</p>
        )}
      </article>
    </div>
  );
