...
async function waitForStandaloneGeneration(
  jobId: string,
  ...
  throw new Error(
    "Tác vụ AI mất nhiều thời gian hơn dự kiến. Vui lòng kiểm tra lại sau.",
  );
}
...
export function ExamsPage({
  ...
  const [pendingGeneratedExam, setPendingGeneratedExam] =
    useState<PendingGeneratedExam | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setExams(
        standaloneOnly
          ? loadedExams.filter(
              (exam) => !exam.courseId && exam.contextType !== "COURSE_QUIZ",
            )
          : loadedExams,
      );
      if (canManage) {
        ...
        setProviders(
          unwrapItems<AiProvider>(providerData as never).filter(
            (item) => item.enabled,
          ),
        );
      }
  ...
  const filtered = useMemo(() => {
    ...
    const result = exams.filter((exam) => {
      ...
      return (
        matchesQuery && (statusFilter === "ALL" || exam.status === statusFilter)
      );
    });
  ...
  async function createExam(event: React.FormEvent<HTMLFormElement>) {
    ...
    setFormError("");
    const data = new FormData(event.currentTarget);
    const courseId = standaloneOnly
      ? ""
      : String(data.get("courseId") ?? "").trim();
    const lessonId = standaloneOnly
      ? ""
      : String(data.get("lessonId") ?? "").trim();
    try {
      ...
      await load();
      window.setTimeout(
        () => window.location.assign(standaloneExamPath(role, created.id)),
        250,
      );
    } catch (caught) {
      setFormError(
  ...
  async function createExamFromDocuments(
    ...
    try {
      ...
      const isSupportedDocument = (file: File) =>
        ...
        file.name.toLocaleLowerCase("vi-VN").endsWith(".pdf") ||
        file.name.toLocaleLowerCase("vi-VN").endsWith(".docx");
      if (sourceFiles.some((file) => !isSupportedDocument(file)))
      ...
      for (const file of sourceFiles) {
        ...
        const stored = await apiRequest<{ id: string }>(
          "/api/v1/files?purpose=QUESTION_SOURCE",
          { method: "POST", body: upload },
        );
        documentFileIds.push(stored.id);
      }
      const createdJob = await apiRequest<GenerationJob>(
      ...
      const job = ["REVIEW_REQUIRED", "APPROVED", "FAILED"].includes(
        ...
        ? createdJob
        : await waitForStandaloneGeneration(createdJob.id);
      ...
      setFormError(
        caught instanceof Error
          ? caught.message
          : "Không thể tạo bài thi từ tài liệu",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function approveGeneratedExamQuestions() {
    ...
    try {
      if (reviewJob.status === "REVIEW_REQUIRED") {
        await apiRequest(
          `/api/v1/ai/question-generation-jobs/${reviewJob.id}/review`,
          {
            ...
            body: JSON.stringify({
              decision: "APPROVE",
              comments: "Giảng viên đã xem trước câu hỏi kỳ thi độc lập",
      ...
      setToast(
        `Đã duyệt ${imported.importedQuestionIds.length} câu và tạo kỳ thi độc lập.`,
      );
      await load();
      window.setTimeout(
        () => window.location.assign(standaloneExamPath(role, createdExam.id)),
        200,
      );
    } catch (caught) {
      setFormError(
        ...
        <div>
          ...
          <p>
            {canManage
              ...
              : "Theo dõi và tham gia các kỳ thi đã được giao cho bạn."}
          </p>
        </div>
        {canManage && (
      ...
      <div className="exam-overview-layout">
        <section className="exam-main-column">
          ...
          {loading ? (
            ...
            <EmptyState
              title="Chưa có bài thi phù hợp"
              description={
                canManage
                  ? "Tạo kỳ thi mới hoặc thay đổi bộ lọc tìm kiếm."
            ...
            <div className="exam-list">
              {filtered.map((exam) => (
                ...
                >
                  <span
                    className={`exam-list-icon status-${exam.status.toLowerCase()}`}
                  >
                    <Icon name="exam" size={22} />
                  </span>
                  <div className="exam-list-primary">
                    <div className="exam-title-line">
                      <h2>{exam.title}</h2>
                    ...
                    <p>
                      {exam.courseId
                        ...
                        : "Kỳ thi độc lập"}
                      <i>•</i>Phiên bản {exam.version}
                    </p>
                    <div className="exam-date-line">
                      <Icon name="clock" size={15} />
                      <span>Mở: {formatDate(exam.opensAt)}</span>
                      <i>•</i>
                      <span>Đóng: {formatDate(exam.closesAt)}</span>
                    </div>
                  </div>
                  ...
                  <span className="exam-row-action" aria-hidden="true">
        ...
        <aside className="exam-side-column">
          <section className="exam-side-card">
            ...
            <dl className="exam-quick-stats">
              <div>
                <dt>Kỳ thi đang mở</dt>
                <dd>{examStats.active}</dd>
              </div>
              <div>
                <dt>Tổng câu hỏi</dt>
                <dd>
                  {exams.reduce((sum, item) => sum + item.questions.length, 0)}
                </dd>
              </div>
              <div>
                <dt>Cần xuất bản</dt>
                <dd>{examStats.draft}</dd>
              </div>
              <div>
                <dt>Tỷ lệ hoạt động</dt>
                <dd>
                  {examStats.total
                    ? Math.round((examStats.active / examStats.total) * 100)
                    : 0}
                  %
                </dd>
              </div>
      ...
      >
        <form className="form-stack" onSubmit={createExamFromDocuments}>
          ...
          <label>
            ...
            />
            <small>
              Có thể chọn nhiều PDF/DOCX. Nội dung cần ở dạng văn bản có thể
              trích xuất.
            </small>
          </label>
          <label>
            ...
            >
              <option value="">Chọn cấu hình AI</option>
          ...
          {providers.length === 0 && (
            <div className="form-alert info">
              <Icon name="warning" />
              Quản trị viên cần cấu hình và bật dịch vụ AI trước.
            </div>
          )}
          {formError && (
          ...
          <div className="modal-actions">
            <button
              type="button"
              className="button secondary"
              onClick={() => setDocumentExamModal(false)}
            >
              Hủy
            </button>
            <button
            ...
            >
              {saving ? "AI đang đọc tài liệu..." : "Sinh câu hỏi để xem trước"}
            </button>
          </div>
        </form>
      </Modal>
          ...
          <>
            ...
            <div className="modal-actions">
              ...
              <button
                type="button"
                className="button primary"
                disabled={saving || !reviewSelected.length}
                onClick={() => void approveGeneratedExamQuestions()}
              >
                {saving
                  ? "Đang nhập câu hỏi..."
      ...
      >
        <form className="form-stack" onSubmit={createExam}>
          <label>
            ...
            <input name="title" required />
            <small>
              Bài thi độc lập không thuộc khóa học. Bài kiểm tra khóa học được
              tạo trong trang chi tiết khóa học.
            </small>
          </label>
          <div className="form-grid four">
            <label>
  ...
  return (
    <form className="form-stack" onSubmit={onSubmit}>
      ...
      <label>
        ...
        <small>Mỗi ô là một phương án. Dùng nút + hoặc − để thêm và xóa.</small>
        <RepeatableField
          name="options"
      ...
      <label>
        ...
        <RepeatableField
          name="correctAnswers"
          initialValues={
            question?.correctAnswers.length ? question.correctAnswers : [""]
          }
