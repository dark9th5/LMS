...
import type { Course, Exam, Lesson, Question } from "@/lib/models";
...
export function ExamsPage({ user }: { user: PortalUser }) {
  ...
  const [examLessons, setExamLessons] = useState<Lesson[]>([]);
  ...
  const filtered = exams.filter((exam) =>
    `${exam.title} ${courseMap.get(exam.courseId ?? "")?.name ?? ""}`
      .toLowerCase()
  ...
  async function createExam(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setToast(
        courseId
          ? "Đã tạo bài kiểm tra trong khóa học."
          : "Đã tạo kỳ thi độc lập ngoài khóa học.",
      );
      await load();
      window.setTimeout(
        () => window.location.assign(`/exams/${created.id}`),
        250,
      );
    } catch (caught) {
      setFormError(
  ...
  async function selectExamCourse(courseId: string) {
    setExamCourseId(courseId);
    setExamLessons([]);
    if (!courseId) return;
    try {
      const course = await apiRequest<Course>(`/api/v1/courses/${courseId}`);
      setExamLessons(
      ...
      setFormError(
        caught instanceof Error
          ? caught.message
          : "Không thể tải bài học của khóa học",
      );
    }
  }
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          canManage
            ? "Bài kiểm tra nằm trong khóa học; kỳ thi độc lập không gắn khóa học. Cả hai dùng chung ngân hàng câu hỏi, tự chấm trắc nghiệm và giữ lịch sử phiên bản đề."
            : "Làm bài kiểm tra trong khóa học hoặc tham gia kỳ thi độc lập đã được cấp quyền."
        }
        icon="exam"
        actions={
          canManage ? (
      ...
      <section className="toolbar-card">
        <label className="search-field">
          ...
          <input
            ...
            placeholder="Tìm bài kiểm tra hoặc kỳ thi"
          />
        </label>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={
            canManage
              ? "Tạo câu hỏi trong ngân hàng, sau đó tạo bài kiểm tra và gắn với khóa học."
              : "Bài kiểm tra sẽ xuất hiện khi khóa học được giao có bài thi đang hoạt động."
          }
        />
      ) : (
          ...
          {filtered.map((exam, index) => (
            ...
            >
              ...
              <div className="exam-card-body">
                ...
                <p>
                  {exam.courseId
                    ? (courseMap.get(exam.courseId)?.name ?? "Khóa học")
                    : "Kỳ thi độc lập"}
                </p>
                <dl>
                  <div>
      ...
      <Modal
        ...
        title="Tạo bài kiểm tra hoặc kỳ thi"
        description="Chọn một khóa học để tạo bài kiểm tra; để trống để tạo kỳ thi độc lập."
      >
        <form className="form-stack" onSubmit={createExam}>
          <div className="form-grid two">
            ...
            <label>
              ...
              <select
                name="courseId"
                value={examCourseId}
                onChange={(event) => void selectExamCourse(event.target.value)}
              >
                <option value="">Kỳ thi độc lập — không thuộc khóa học</option>
                {courses.map((course) => (
          ...
          {examCourseId && (
            <label>
              Bài học kiểm tra <b>*</b>
              <select
                name="lessonId"
                required
                disabled={examLessons.length === 0}
              >
                <option value="">Chọn bài học loại EXAM</option>
              ...
              <small>
                {examLessons.length
                  ? "Kết quả đạt sẽ tự động hoàn thành đúng bài học trong lớp."
                  : "Khóa học chưa có bài học loại EXAM. Hãy bổ sung bài học trước khi tạo đề."}
              </small>
            </label>
          )}
          ...
          <div className="modal-actions">
            ...
            <button
              ...
              disabled={
                ...
                Boolean(examCourseId && examLessons.length === 0)
              }
            >
              {saving ? "Đang tạo..." : "Tạo bài kiểm tra / kỳ thi"}
            </button>
          </div>
        </form>
      </Modal>
