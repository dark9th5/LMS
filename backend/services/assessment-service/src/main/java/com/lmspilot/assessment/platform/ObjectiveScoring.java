...
public final class ObjectiveScoring {
    ...
    public static ObjectiveScore score(QuestionType type,List<String> expected,JsonNode answer,double points){
        ...
        expected.forEach(v->exp.add(normal(v)));
        Set<String> got=new LinkedHashSet<>();
        if(answer!=null){
            if(answer.isArray()) answer.forEach(n->got.add(normal(n.asText())));
            else got.add(normal(answer.asText()));
        }
        if(exp.equals(got)) return new ObjectiveScore(points,points,true);
        if(type==QuestionType.MULTIPLE_CHOICE && !got.isEmpty() && exp.containsAll(got)){
            double ratio=(double)got.size()/Math.max(1,exp.size());
            return new ObjectiveScore(points*ratio,points,false);
        }
