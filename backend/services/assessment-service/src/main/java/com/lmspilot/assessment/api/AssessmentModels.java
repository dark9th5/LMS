...
public final class AssessmentModels {
    ...
    public record QuestionRequest(QuestionType type,@NotBlank String prompt,List<String> options,List<String> correctAnswers,String explanation,@Min(1)
    @Max(5) int difficulty,Set<String> tags,@DecimalMin(value="0.0",inclusive=false) double defaultPoints){
        public QuestionRequest {
            options=options==null?List.of():options;
            correctAnswers=correctAnswers==null?List.of():correctAnswers;
    ...
    public record ExamRequest(@NotBlank
    @Size(max=220) String title,UUID courseId,UUID lessonId,AssessmentContextType contextType,UUID cohortId,boolean autoGrade,@Min(1)
    @Max(480) int durationMinutes,Instant opensAt,Instant closesAt,@Min(1)
    @Max(20) int maxAttempts,@Min(0) int waitMinutesBetweenAttempts,@DecimalMin("0.0")
    @DecimalMax("100.0") double passingScore,boolean shuffleQuestions,boolean shuffleAnswers,ScoreStrategy scoreStrategy,ExamStatus status,List<ExamQuestionInput> questions){
    }
    ...
    public record SaveAnswersRequest(Map<String,JsonNode> answers){
