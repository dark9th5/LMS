...
@NotBlank
@Size(max = 240) String titleTemplate,
...
@Min(-365)
@Max(365) Integer daysBeforeDue,
@Min(0)
@Max(23) Integer hourUtc,
Boolean enabled
) {
}
...
public class NotificationAutomationApi {
    ...
    public NotificationAutomationApi(NotificationAutomationService service) {
    ...
    public NotificationTemplateResponse create(@Valid
    @RequestBody NotificationTemplateRequest input) {
        return service.saveTemplate(null, input);
    }
    @PutMapping("/{id}")
    public NotificationTemplateResponse update(@PathVariable UUID id, @Valid
    @RequestBody NotificationTemplateRequest input) {
...
class NotificationReminderController {
    private final NotificationAutomationService service;
    NotificationReminderController(NotificationAutomationService service) {
        this.service = service;
    }
    @GetMapping
    ...
    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    NotificationReminderRuleResponse create(@Valid
    @RequestBody NotificationReminderRuleRequest input) {
        return service.saveRule(null, input);
    }
    @PutMapping("/{id}")
    NotificationReminderRuleResponse update(@PathVariable UUID id, @Valid
    @RequestBody NotificationReminderRuleRequest input) {
        return service.saveRule(id, input);
    }
    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
