...
public interface LessonProgressRepository extends JpaRepository<LessonProgressEntity, UUID> {
    ...
    List<LessonProgressEntity> findAllByEnrollmentIdOrderByUpdatedAtAsc(UUID enrollmentId);
    List<LessonProgressEntity> findAllByEnrollmentIdInOrderByUpdatedAtAsc(Collection<UUID> enrollmentIds);
}
