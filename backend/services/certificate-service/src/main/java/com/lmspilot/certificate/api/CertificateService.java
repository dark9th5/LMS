...
public class CertificateService {
    ...
    public String printable(UUID id) {
        ...
        try {
            template = mapper.readValue(certificate.getTemplateSnapshotJson(), CertificateTemplateSnapshot.class);
        }
        catch (Exception exception) {
            template = defaults();
        }
    ...
    private String nextCode() {
        ...
        attempt++) {
            ...
            for (int index = 0;
            index < 16;
            index++) raw.append(alphabet.charAt(random.nextInt(alphabet.length())));
    ...
    private static ApiException notFound() {
        return new ApiException(HttpStatus.NOT_FOUND, "CERTIFICATE_NOT_FOUND", "Không tìm thấy chứng chỉ");
    }
