...
public interface OrganizationUnitRepository extends JpaRepository<OrganizationUnitEntity,UUID>{
 ...
 @Query("select o from OrganizationUnitEntity o where (cast(:query as string) is null or lower(o.code) like lower(concat('%', cast(:query as string), '%')) or lower(o.name) like lower(concat('%', cast(:query as string), '%'))) and (:status is null or o.status = :status) order by o.materializedPath, o.sortOrder, o.name")
