...
public class AuditController {
    ...
    @RequestParam(required=false)Instant from,@RequestParam(required=false)Instant to,@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="50")int size){
        return service.search(actor,action,resourceType,resourceId,outcome,correlationId,from,to,page,size);
    }
    @GetMapping("/export.csv")
    @PreAuthorize("hasAuthority('"+Permissions.AUDIT_EXPORT+"')")
