...
public final class IdentityModels { private IdentityModels(){}
 ...
 public record TokenResponse(String accessToken,String refreshToken,String tokenType,long expiresInSeconds,UserSummary user){ public TokenResponse(String access,String refresh,long ttl,UserSummary user){this(access,refresh,"Bearer",ttl,user);} }
 ...
 public record CreateUserRequest(@NotBlank @Size(max=80) String code,@NotBlank @Size(min=3,max=120) String username,@NotBlank @Size(min=12,max=128) String password,@NotBlank @Size(max=180) String fullName,@Email String email,UUID organizationUnitId,@Size(min=1,max=1) Set<String> roleCodes,boolean mustChangePassword){}
 public record BulkCreateUsersRequest(@NotBlank @Size(max=120) String operationId,@Valid @Size(min=1,max=1000) List<CreateUserRequest> users){}
 public record BulkCreateUsersResponse(String operationId,List<UserSummary> created){}
 public record UpdateUserRequest(@NotBlank @Size(max=180) String fullName,@Email String email,UUID organizationUnitId,@Size(min=1,max=1) Set<String> roleCodes,AccountStatus status){}
 public record ResetPasswordRequest(@NotBlank @Size(min=12,max=128) String newPassword,boolean forceChangeOnNextLogin){}
 ...
 public record RoleRequest(@NotBlank @Size(max=80) String code,@NotBlank @Size(max=160) String name,Set<String> permissions){}
 public record RoleResponse(UUID id,String code,String name,Set<String> permissions,boolean systemRole){}
 public record GrantInput(String roleCode,String permissionCode,ScopeType scopeType,UUID scopeId,GrantEffect effect,Instant validFrom,Instant validUntil){ public boolean hasExactlyOneSubject(){return (roleCode==null)^(permissionCode==null);} public boolean validScope(){return scopeType==ScopeType.SYSTEM?scopeId==null:scopeId!=null;} }
 public record BulkGrantRequest(@NotBlank @Size(max=120) String operationId,@Size(min=1,max=1000) Set<UUID> userIds,@Valid @Size(min=1,max=100) List<GrantInput> grants){}
 ...
 public record RevokeGrantsRequest(@NotBlank @Size(max=120) String operationId,@Size(max=5000) Set<UUID> grantIds,@Size(max=5000) Set<UUID> roleAssignmentIds){ public RevokeGrantsRequest{grantIds=grantIds==null?Set.of():grantIds;roleAssignmentIds=roleAssignmentIds==null?Set.of():roleAssignmentIds;} public boolean hasTarget(){return !grantIds.isEmpty()||!roleAssignmentIds.isEmpty();} }
 public record RevokeGrantsResponse(long permissionGrantsDeleted,long roleAssignmentsDeleted){}
 public record PageResponse<T>(List<T> items,int page,int size,long totalElements,int totalPages){}
 public record UserImportMappingRequest(String codeColumn,String usernameColumn,String fullNameColumn,String emailColumn,String organizationUnitIdColumn,String roleCodesColumn,String passwordColumn,String statusColumn,Set<String> defaultRoleCodes,String defaultPassword,UserImportMode mode,UserImportFailurePolicy failurePolicy,boolean updatePasswordOnUpsert){ public UserImportMappingRequest{codeColumn=blank(codeColumn,"code");usernameColumn=blank(usernameColumn,"username");fullNameColumn=blank(fullNameColumn,"fullName");defaultRoleCodes=defaultRoleCodes==null||defaultRoleCodes.isEmpty()?Set.of("STUDENT"):defaultRoleCodes;mode=mode==null?UserImportMode.CREATE_ONLY:mode;failurePolicy=failurePolicy==null?UserImportFailurePolicy.PARTIAL:failurePolicy;} private static String blank(String v,String d){return v==null||v.isBlank()?d:v;} }
 ...
 public record BulkGrantPreviewRequest(@Size(min=1,max=1000) Set<UUID> userIds,@Valid @Size(min=1,max=100) List<GrantInput> grants){}
