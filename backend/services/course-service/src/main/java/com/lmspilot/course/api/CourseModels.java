...
record CategoryRequest(@NotBlank
@Size(max=80) String code,@NotBlank
@Size(max=180) String name,UUID parentId,Integer sortOrder) {
}
record CategoryResponse(UUID id,String code,String name,UUID parentId,RecordStatus status,int sortOrder) {
}
record CourseRequest(@NotBlank
@Size(max=80) String code,@NotBlank
@Size(max=240) String name,String description,String objectives,String targetAudience,@Min(0) Integer durationMinutes,@DecimalMin("0")
@DecimalMax("100") Double passingScore,String completionPolicyJson,UUID categoryId) {
}
record LessonRequest(@NotBlank
@Size(max=220) String title,@NotNull LessonType type,String textContent,UUID fileId,Boolean required,@Min(0) Integer sortOrder,@Min(0) Integer estimatedMinutes) {
}
